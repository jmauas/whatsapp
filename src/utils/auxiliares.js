...
export const formatoFecha = (sfecha, hs, sec, americana, dia) => {
    if (sfecha != undefined) {
		let fh;
        try {
...
const ensureDirectoryExists = (dirPath) => {
    if (!fs.existsSync(dirPath)) {
      ...
      console.log(new Date().toLocaleString()+'  -  '+`Carpeta creada: ${dirPath}`);
    } else {
      console.log(new Date().toLocaleString()+'  -  '+`La carpeta ya existe: ${dirPath}`);
    }
  };

export const crearCarpetasTemporales = () => {
    const targetDir = path.join(os.tmpdir());
